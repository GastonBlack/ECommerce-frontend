...
export default function CartPage() {
  ...
  const loadCart = async () => {
    ...
    try {
      ...
      else setError("No se pudo cargar el carrito.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const updateQty = async (cartItemId: number, newQty: number) => {
    ...
    try {
      ...
      if (err.response?.status === 401) router.push("/login");
      else setError("No se pudo actualizar la cantidad.");
    } finally {
      setBusyId(null);
    }
  };
  ...
  const removeItem = async (cartItemId: number) => {
    ...
    try {
      ...
      if (err.response?.status === 401) router.push("/login");
      else setError("No se pudo eliminar el producto del carrito.");
    } finally {
      setBusyId(null);
    }
  };
  ...
  const clearCart = async () => {
    ...
    try {
      ...
      if (err.response?.status === 401) router.push("/login");
      else setError("No se pudo vaciar el carrito.");
    } finally {
      setBusyId(null);
    }
  };
